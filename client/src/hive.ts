...
export interface Game {
    move: number
    colorToMove: Color
    whiteReserve: [number, number, number, number, number, number, number]
    blackReserve: [number, number, number, number, number, number, number]
    tiles: {
        position: {
            q: number,
            r: number,
...
export function legalPlacements(tileIndex: number) {
...
export function legalMovements(tileIndex: number) {
    console.assert(gamePtr > 0)
